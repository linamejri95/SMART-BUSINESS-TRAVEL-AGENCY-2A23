...
client_workspace::client_workspace(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::client_workspace)
{
    ui->setupUi(this);
...
void client_workspace::update_label()
{
...
ui->ajouter_client->setCurrentIndex(7);//fenetre 7 tableau
...
void client_workspace::on_pushButton_2_clicked()
{
...
void client_workspace::on_bouton_supp_clicked()
{
    Client c;
    ...
    if(test)
    {
        ...
        ui->le_tab->setModel(c.afficher());
         ui->comboBox->setModel(c.tester());
          ui->comboBox_2->setModel(c.tester());
          addToHistory("suppression","d'un client ",ui->comboBox->currentText());
          ui->listView->setModel(afficherHistorique());
...
void client_workspace::on_pushButton_clicked()
{
    int id;
...
void client_workspace::on_comboBox_2_currentIndexChanged(const QString &arg1)
{
...
void client_workspace::on_pushButton_3_clicked()
{
    statistique *client_workspace=new statistique(this);
                  client_workspace->show();
}




void client_workspace::on_pushButton_4_clicked()
{
    QString strStream;
...
void client_workspace::on_pushButton_5_clicked()
{
    PieChartWidget *client_workspace=new PieChartWidget(this);
                  client_workspace->show();
}

void client_workspace::on_triii_currentIndexChanged(const QString &arg1)
{
...
void client_workspace::on_rechercher_clicked()
{
...
void client_workspace::on_rech_textChanged(const QString &arg1)
{
...
void client_workspace::on_excelll_2_textChanged(const QString &arg1)
{
...
void client_workspace::on_excelll_clicked()
{
...
void client_workspace::addToHistory(QString action, QString type, QString id)
{
    ...
    activity="\n    "+date1+"   -   "+time+" \t    "+ action +" "+type+" d'identifiant:  "+id+" \n";
          ...
          query.prepare("INSERT INTO HISTORY_(activity, DATE_ACTIVITE_HISTORIQUE) VALUES (:activity, :sysdate)");
...
QSqlQueryModel* client_workspace::afficherHistorique()
{
    ...
    //qDebug() << "test";
         query.prepare("SELECT activity FROM HISTORY_ where activity like '%"+historyType+"%'  order by date_activite_historique desc");
         query.exec();
...
bool client_workspace::historyDelete()
{
    QSqlQuery query;
    int periode;
    ...
    query.prepare("Delete from HISTORY_ where date_activite_historique > :lastdate ");
...
void client_workspace::on_history_activated(const QString &arg1)
{
     ui->listView->setModel(afficherHistorique());
}


void client_workspace::on_del_clicked()
{
...
void client_workspace::on_deleteee_activated(const QString &arg1)
{
    ui->listView->setModel(afficherHistorique());
}

void client_workspace::on_listView_indexesMoved(const QModelIndexList &indexes)
{
...
void client_workspace::on_son_clicked()
{
...
void client_workspace::on_start_clicked()
{
mMediaPlayer->play();
}


void client_workspace::on_stop_clicked()
{
    mMediaPlayer->stop();
}


void client_workspace::on_mute_clicked()
{
...
void client_workspace::on_horizontalSlider_valueChanged(int valeur)
{
    mMediaPlayer->setVolume(valeur);
}

void client_workspace::on_message_clicked()
{
    Smtp* smtp = new Smtp("nourchene.laroussi@esprit.tn","211JFT0306", "smtp.gmail.com");
            //connect(smtp, SIGNAL(status(QString)), this, SLOT(mailSent(QString)));


                smtp->sendMail("nourchene.laroussi@esprit.tn", "nourchene.laroussi@esprit.tn" , ui->message->text()," ");
}



void client_workspace::on_ouvrir_clicked() //sortir
{
    c.incrParking();
                   data2.clear();
                   data2.append("5");
...
void client_workspace::on_ouvrir_2_clicked() //entrer
{
    c.decrParking();
        data2.clear();
        data2.append("6");
...
void client_workspace::on_pushButton_6_clicked()
{
    close();
