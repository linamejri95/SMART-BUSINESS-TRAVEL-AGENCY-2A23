...
{
    QSqlQuery q2;
    q2.prepare("select count(id_loc)from LOC")  ;
            ...
            regen->setName("Nombre de LOC par type");
            ...
            ticks << 1 << 2 << 3 << 4 << 5 ;
            QSqlQuery q;
            q.prepare("select count(id_loc), type from LOC group by type")  ;
            ...
            ui->customPlot->yAxis->setLabel("Nombre LOC");
